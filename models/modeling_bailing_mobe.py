...
from models.configuration_bailing_moe import BailingMoeConfig
...
class BailingMoBEMLP(nn.Module):
...
class BailingMoBESparseMoeBlock(nn.Module):
    """
    ...
    def __init__(self, config: BailingMoeConfig):
        ...
        self.W_up = nn.Parameter(torch.zeros(config.num_experts, config.num_B))

        self.B_gate = nn.Parameter(torch.zeros(config.num_B, config.moe_intermediate_size, config.hidden_size))
        self.W_gate = nn.Parameter(torch.zeros(config.num_experts, config.num_B))
    ...
    def _setup_experts(self):
        self.experts = nn.ModuleList(
            [
                BailingMoBEMLP(config=self.config, intermediate_size=self.config.moe_intermediate_size)
...
class BailingMoeDecoderLayer(nn.Module):
    def __init__(self, config: BailingMoeConfig, layer_idx: int):
        ...
        self.mlp = (
            BailingMoBESparseMoeBlock(config)
...
class BailingMoBEForCausalLM(BailingMoePreTrainedModel):
